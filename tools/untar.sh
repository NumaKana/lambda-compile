...
if ! [ -e "lilypond-${version}.linux-64.tar.bz2" ]; then
  if ! [ -e ${version}.sh ]; then
    echo
    echo '>>> Downloading'
    echo ">>> curl -o ${version}.sh http://lilypond.org/download/binaries/linux-64/lilypond-${version}.linux-64.sh"
    echo
    sudo curl -o ${version}.sh http://lilypond.org/download/binaries/linux-64/lilypond-${version} .linux-64.sh
  fi
  ...
  echo
  sudo sh ${version}.sh --tarball --batch
fi
...
echo
sudo tar -xjf lilypond-${version}.linux-64.tar.bz2 -C "$output"
